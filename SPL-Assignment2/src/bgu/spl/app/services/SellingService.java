...
/**
 * This micro-service handles PurchaseOrderRequests. When the SellingService receives a PurchaseOrderRequest,
...
public class SellingService extends MicroService{
	...
	/**
	 * This method handles TickBroadcasts, PurchaseOrderRequests and TerminationBroadcasts.
	...
	/**
	 * This method handles the PurchaseOrderRequests received from the WebsiteClients.
	 * It attempts to take the shoes from the store.
	 * If It succeded, it 
	 * 
	 */
	private void subscribeToPurchaseOrderRequest(){
		subscribeRequest(PurchaseOrderRequest.class, purchaseRequest -> {
			log.log(Level.INFO, " has received a purchase request for "+purchaseRequest.getAmountSold()+" shoes of type "+purchaseRequest.getShoeType());
			...
			if(res.equals(BuyResult.DISCOUNTED_PRICE) || res.equals(BuyResult.REGULAR_PRICE)){ 
				/* Create a new receipt for purchasing from the seller and complete
				 * the purchase request with it as it's result */
				...
				Receipt receipt = new Receipt(this.getName(), purchaseRequest.getCustomer(), purchaseRequest.getShoeType(), retreivedWithDiscount, currentTick, purchaseRequest.getRequestTick(), purchaseRequest.getAmountSold());
				...
				log.log(Level.INFO, getName()+" couldn't complete the purchase request because there were no discounted items on stock as requested ");
				complete(purchaseRequest,null);
			}
			else if(res.equals(BuyResult.NOT_IN_STOCK)){
				/* There weren't enough shoes in the store, send a RetockRequest to the manager */
				RestockRequest requestToRestock = new RestockRequest(purchaseRequest.getShoeType(),1, purchaseRequest.getRequestTick());
				sendRequest(requestToRestock, isRestockRequestSuccessful -> {
					log.log(Level.INFO, getName()+" has sent a restock request");
					/* If the restock request was successful, create a new receipt and complete the original 
					  purchase request with it. Otherwise complete it with null to indicate failure*/
					if(isRestockRequestSuccessful){
						Receipt receipt = new Receipt(this.getName(), purchaseRequest.getCustomer(), purchaseRequest.getShoeType(), purchaseRequest.isDiscount(), currentTick, purchaseRequest.getRequestTick(), purchaseRequest.getAmountSold());
						storeInstance.file(receipt);
						log.log(Level.INFO, getName()+" has completed the purchase order for "+purchaseRequest.getAmountSold()+" shoes of type "+purchaseRequest.getShoeType()+" which was issued originaly at tick: "+purchaseRequest.getRequestTick()+" by the customer "+purchaseRequest.getCustomer());
						complete(purchaseRequest, receipt); 
					}
					else{
						log.log(Level.WARNING, getName()+" couldn't complete the purchase order for "+purchaseRequest.getAmountSold()+" shoes of type "+purchaseRequest.getShoeType()+" which was issued originally at tick:"+purchaseRequest.getRequestTick()+" by the customer "+purchaseRequest.getCustomer()+" becasue the restock request failed");
						complete(purchaseRequest, null);
					}
				});
			}
	...
	/** 
	 * This method handles TerminationBroadcasts, by starting a graceful termination of the SellingService. 
	 * Firstly It subscribes to TerminationBroadcasts. When a new termination broadcast is received,
	 * we invoke the end latch countDown to indicate the SellingService is terminating,
