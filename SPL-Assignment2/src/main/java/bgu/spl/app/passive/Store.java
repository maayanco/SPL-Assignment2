...
/**
 * The store is a thread - safe Singleton class. It holds a collection of
 * ShoeStorageInfo: One for each shoe type the store offers. In addition, it
 * contains a list of receipts issued to and by the store.
 */
public class Store {
	...
	/**
	 * Receives an array of ShoeStorageInfo items, and adds them to the shoe
	 * storage list
	 * 
	 * @param storage
	 *            - array of ShoeStorageInfo
	 */
	public void load(ShoeStorageInfo[] storage) {
		...
		for (ShoeStorageInfo item : storage) {
			...
			addDiscount(item.getShoeType(), item.getDiscountedAmount());
		}
	}

	/**
	 * Adds a new shoe with the shoeType and amount parameters to the shoe
	 * storage list
	 * 
	 * @param shoeType
	 *            - the type of shoe
	 * @param amount
	 *            - the amount of shoes to be on stock
	 */
	...
	/**
	 * Converts shoes of the required shoe type to be on discount. This method
	 * can only convert existing shoes and not add new shoes to the stock.
	 * 
	 * @param shoeType
	 *            - the type of shoes to be on discount
	 * @param amount
	...
	/**
	 * Updates the amount of shoes - according to typeOfOperation it either adds
	 * a new shoe or converts an existing shoe to be on discount.
	 * 
	 * @param shoeType
	 *            - the type of shoe
	 * @param amountToAdd
	 *            - the amount of shoes to add
	 * @param typeOfOperation
	 *            - indicate whether the method should add a new shoe to the
	 *            stock or convert a shoe to be on discount
	 */
	private void updateAmount(String shoeType, int amountToAdd, String typeOfOperation) {
		...
		if (locatedShoe == null) {
			/*
			 * Adding the shoe to the storage, which is synchronized so that no
			 * other thread will perform a modification at the same time
			 */
		...
		synchronized (locatedShoe) {
			/*
			 * We modify the amount of the locatedShoe and therefore we
			 * synchronize the locatedShoe object so that no other thread will
			 * modify this same object
			 */
	 ...
	 * 
	 * @param shoeType
	 *            - the type of the shoe we are searching
	...
	/**
	 * Attempts to locate and return a shoe of the specified parameters, if not
	 * found returns appropriate status message.
	 * 
	 * @param shoeType
	 *            - the type of the shoe to be taken
	 * @param onlyDiscount
	 *            - indicates whether the shoe we want to take should be
	 *            discounted or not
	 ...
	 * 
	 * @param receipt
	 *            - a receipt created after manufacturing or selling of shoes
	 */
