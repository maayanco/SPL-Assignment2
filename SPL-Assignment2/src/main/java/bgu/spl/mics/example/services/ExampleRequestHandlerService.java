...
public class ExampleRequestHandlerService extends MicroService {
	...
	public ExampleRequestHandlerService(String name, String[] args) {
		...
		if (args.length != 1) {
			throw new IllegalArgumentException(
					"Request Handler expecting a single argument: mbt (the number of requests to answer before termination)");
		}
		...
		try {
			...
			throw new IllegalArgumentException(
					"Request Handler expecting the argument mbt to be a number > 0, instead received: " + args[0]);
		}
	...
	protected void initialize() {
		...
		subscribeRequest(ExampleRequest.class, req -> {
			mbt--;
			System.out.println("Request Handler " + getName() + " got a new request from " + req.getSenderName()
					+ "! (mbt: " + mbt + ")");
