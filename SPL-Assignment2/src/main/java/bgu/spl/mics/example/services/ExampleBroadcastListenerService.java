...
public class ExampleBroadcastListenerService extends MicroService {
	...
	public ExampleBroadcastListenerService(String name, String[] args) {
		...
		if (args.length != 1) {
			throw new IllegalArgumentException(
					"Listener expecting a single argument: mbt (the number of requests to answer before termination)");
		}
		...
		try {
			...
			throw new IllegalArgumentException(
					"Listener expecting the argument mbt to be a number > 0, instead received: " + args[0]);
		}
	...
	protected void initialize() {
		...
		subscribeBroadcast(ExampleBroadcast.class, message -> {
			mbt--;
			System.out.println("Listener " + getName() + " got a new message from " + message.getSenderId() + "! (mbt: "
					+ mbt + ")");
			if (mbt == 0) {
